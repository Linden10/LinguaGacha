...
def test_translation_scheduler_handle_failed_context_uses_configured_max_round_for_single_item_retry() -> (
    None
):
    item = create_item("single")
    context = TaskContext(
    ...
    scheduler = TranslationScheduler(
        config=Config(max_round=5, preceding_lines_threshold=2),
        model={"threshold": {"input_token_limit": 64}},
        items=[item],
    )
